...
async def handle_moderation_commands(manager: ConnMgr, ws, sub: str, role: str, txt: str) -> bool:
    ...
    if m and is_adminish:
        target_raw = m.group(1) or m.group(2)
    ...
    # /unmute <user> (quoted or unquoted); admin or dev
    m = re.match(r'^\s*/unmute\s+(?:"([^"]+)"|(\S+))\s*$', txt, re.I)
    if m and is_adminish:
        target_raw = m.group(1) or m.group(2)
        target = canonical_user(manager, target_raw)
    ...
    m = re.match(r'^\s*/locktag\s+(?:"([^"]+)"|(\S+))\s*$', txt, re.I)
    if m:
    ...
    m = re.match(r'^\s*/unlocktag\s+(?:"([^"]+)"|(\S+))\s*$', txt, re.I)
    if m:
        if not is_dev(manager, sub):
