...
export default function UnbanModal({ open, banned, onClose, onSubmit }: Props) {
  const [selected, setSelected] = useState<Record<string, boolean>>({});
  const primaryRef = useRef<HTMLButtonElement | null>(null);

  const candidates = useMemo(() => (Array.isArray(banned) ? banned : []), [banned]);
  ...
  return (
    <div className="fixed inset-0 z-[160] flex items-center justify-center">
      ...
      <form onSubmit={(e)=>{e.preventDefault(); submit();}} className="relative w-[min(92vw,560px)] max-h-[88vh] overflow-hidden bg-black/90 border border-white/30 rounded-3xl shadow-2xl text-[#f7f3e8] p-0 flex flex-col animate-[modal-in_140ms_ease-out]">
        <div className="text-center text-white text-lg font-semibold py-4">Unban Users</div>
        <hr className="border-white/10" />
        <div className="p-4 overflow-y-auto max-h-[60vh]">
          {candidates.length === 0 ? (
            <div className="text-sm text-white/70 text-center py-6">No users are currently banned.</div>
          ) : (
